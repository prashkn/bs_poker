...
func newTestRoom(numPlayers int) *models.Room {
	room := &models.Room{
		ID: "test-room",
		...
		Players: make([]*models.Player, 0, numPlayers),
	}
...
func TestVerifyClaim_Valid(t *testing.T) {
	allCards := []models.Card{
		{Suit: models.Hearts, Value: models.Ace},
		{Suit: models.Spades, Value: models.Ace},
		{Suit: models.Clubs, Value: models.King},
		{Suit: models.Diamonds, Value: models.Queen},
	}
	claim := models.MadeHand{
		HandType: models.Pair,
		Cards: []models.Card{
			{Suit: models.Hearts, Value: models.Ace},
			{Suit: models.Spades, Value: models.Ace},
		},
...
func TestVerifyClaim_Invalid(t *testing.T) {
	...
	claim := models.MadeHand{
		...
		Cards: []models.Card{
			{Suit: models.Hearts, Value: models.Ace},
			{Suit: models.Spades, Value: models.Ace}, // not in pool
		},
	}
	if VerifyClaim(claim, allCards) {
...
func TestStartGame_Success(t *testing.T) {
	...
	if room.Game.State != models.StateTurn {
		t.Errorf("expected StateTurn, got %d", room.Game.State)
	}
	if room.Game.Round != 1 {
		t.Errorf("expected round 1, got %d", room.Game.Round)
	}
	if len(room.Game.TurnOrder) != 3 {
		t.Errorf("expected 3 players in turn order, got %d", len(room.Game.TurnOrder))
	}
...
func TestMakeClaim_Success(t *testing.T) {
	...
	currentPlayerID := room.Game.TurnOrder[room.Game.CurrentTurn]
	claim := models.MadeHand{
		HandType: models.HighCard,
		Cards:    []models.Card{{Suit: models.Hearts, Value: models.Ace}},
	}
	...
	if room.Game.CurrentClaim == nil {
		t.Fatal("current claim should be set")
	}
	if room.Game.CurrentClaim.PlayerID != currentPlayerID {
...
func TestMakeClaim_MustBeStronger(t *testing.T) {
	...
	p1 := room.Game.TurnOrder[room.Game.CurrentTurn]
	claim1 := models.MadeHand{
		HandType: models.Pair,
		Cards: []models.Card{
			{Suit: models.Hearts, Value: models.Ace},
			{Suit: models.Spades, Value: models.Ace},
	...
	p2 := room.Game.TurnOrder[room.Game.CurrentTurn]
	claim2 := models.MadeHand{
		HandType: models.HighCard,
		Cards:    []models.Card{{Suit: models.Hearts, Value: models.King}},
	}
...
func TestCallBS_ClaimInvalid(t *testing.T) {
	...
	// Make a claim that cannot possibly exist in the dealt cards
	p1 := room.Game.TurnOrder[room.Game.CurrentTurn]
	// Claim three aces — but each player only has 2 cards, and it's unlikely all 3 aces are there
	// Instead, let's set up hands explicitly
	room.Players[0].Hand = []models.Card{
		{Suit: models.Hearts, Value: models.Two},
		{Suit: models.Clubs, Value: models.Three},
	}
	room.Players[1].Hand = []models.Card{
		{Suit: models.Diamonds, Value: models.Four},
		{Suit: models.Spades, Value: models.Five},
	}
...
func TestCallBS_Elimination(t *testing.T) {
	...
	// Player 0 has already lost once (card count = 2 from start, will go to 3 on loss)
	// Actually CardCount starts at 2, so one loss → 3 = MaxCards → eliminated
	_ = StartGame(room, room.HostID)
...
func TestCallBS_NewRound(t *testing.T) {
	...
	room.Players[2].Hand = []models.Card{
		{Suit: models.Hearts, Value: models.Six},
		{Suit: models.Clubs, Value: models.Seven},
	}
	...
	if room.Game.Round != 2 {
		t.Errorf("expected round 2, got %d", room.Game.Round)
	}
	if room.Game.CurrentClaim != nil {
